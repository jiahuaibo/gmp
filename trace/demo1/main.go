...
func createSeq_1(width, height int) image.Image {
	m := image.NewGray(image.Rect(0, 0, width, height))
...
func createSeq_2(width, height int) image.Image {
	m := image.NewGray(image.Rect(0, 0, width, height))
	var w sync.WaitGroup
	w.Add(width)
	for i := 0; i < width; i++ {
...
func createSeq_3(width, height int) image.Image {
	m := image.NewGray(image.Rect(0, 0, width, height))
...
func createSeq_4(width, height int) image.Image {
	m := image.NewGray(image.Rect(0, 0, width, height))
